...
import {
  ...
  Home, UserPlus, List, SlidersHorizontal, ArrowUpDown
} from "lucide-react";
import {
...
const withinNextDays = (d, days) => { if (!d) return false; const n = daysUntil(d); return n >= 0 && n <= days; };
...
function runSelfTests() {
  try {
    ...
    const kgb = "2023-08-01"; const pangkat = "2022-01-10";
    console.assert(ymd(addYears(toDate(kgb), 2)) === "2025-08-01");
    console.assert(ymd(addYears(toDate(pangkat), 4)) === "2026-01-10");
  } catch {}
}
      ...
      <header className="sticky top-0 z-40 backdrop-blur bg-white/75 border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-semibold leading-tight">Monitoring Jadwal Kenaikan Pangkat Berikutnya & Kenaikan Gaji Berikutnya (ASN)</h1>
            <p className="text-xs text-slate-500 -mt-0.5">Pantau Kenaikan Gaji Berikutnya & Kenaikan Pangkat Berikutnya secara proaktif</p>
          </div>
...
function FormInput() {
  const [form, setForm] = useState({
    nama: "", nip: "", tmtPns: "", riwayatTmtKgb: "",
    riwayatTmtPangkat: "", jadwalKgbBerikutnya: "", jadwalPangkatBerikutnya: "",
  });
    ...
    <div className="grid grid-cols-1 gap-6">
      <Card title="Input Data Pegawai" subtitle="Lengkapi data berikut. Jadwal otomatis dihitung.">
        <form onSubmit={submit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormRow label="Nama" required><input name="nama" value={form.nama} onChange={onChange} className="w-full border rounded-lg px-3 py-2" placeholder="Nama Lengkap" /></FormRow>
          <FormRow label="Nomor Pegawai (NIP)" required><input name="nip" value={form.nip} onChange={onChange} className="w-full border rounded-lg px-3 py-2" placeholder="1985xxxxxxxxxxxx" /></FormRow>
...
function TabelData() {
  ...
  const filtered = useMemo(() => {
    ...
    let list = withMeta;
    if (term) list = list.filter((r) => r.nama?.toLowerCase().includes(term) || (r.nip || "").toLowerCase().includes(term));
    ...
    <Card title="Tampilkan Data Pegawai" subtitle="Cari, filter, dan urutkan data pegawai." extra={
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Cari nama atau NIP..." className="border rounded-lg pl-9 pr-3 py-2 w-72 max-w-full" />
        </div>
        ...
        <table className="min-w-full">
          <thead className="sticky top-0 z-10">
            <tr className="bg-white/95 backdrop-blur text-slate-600 border-b">
              <Th>Nama</Th><Th>NIP</Th><Th>TMT PNS</Th><Th>Riwayat TMT Kenaikan Gaji</Th><Th>Jadwal Kenaikan Gaji Berikutnya</Th><Th>Riwayat TMT Pangkat</Th><Th>Jadwal Kenaikan Pangkat Berikutnya</Th><Th>Status</Th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((r, idx) => (
            ...
            {!filtered.length && <tr><td className="p-8 text-center text-slate-500" colSpan={8}>Belum ada data.</td></tr>}
          </tbody>
        </table>
      </div>
    </Card>
...
// Notifikasi page WITH TelegramBox
function PanelNotifikasi() {
  ...
  return (
    <div className="grid grid-cols-1 gap-6">
      <TelegramBox soon={notif.soon} overdue={notif.overdue} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {TYPES.map((type) => (
          <Card key={type} title={type} subtitle="Dikelompokkan per status">
            <div className="space-y-5">
              <div><div className="text-xs font-medium text-amber-700 mb-2">Segera (≤3 bln)</div><NotifList items={byJenis(notif.soon, type)} tone="amber" emptyText="Tidak ada yang jatuh tempo." /></div>
              <div><div className="text-xs font-medium text-rose-700 mb-2">Terlewat</div><NotifList items={byJenis(notif.overdue, type)} tone="rose" overdue emptyText="Tidak ada yang terlewat." /></div>
            </div>
          </Card>
        ))}
      </div>
...
// ==== Telegram client helpers & UI =====
function escapeHtml(str = "") {
  return String(str).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;");
}
function formatDigest({ soon = [], overdue = [] } = {}) {
  const fmtDate = (d) => new Date(d).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" });
  const line = (r) => `• <b>${escapeHtml(r.nama)}</b> (${escapeHtml(r.nip || "-")}) — ${escapeHtml(r.jenis)}: <b>${fmtDate(r.tanggal)}</b>` + (new Date(r.tanggal) < new Date() ? ` (terlewat ${Math.abs(daysUntil(r.tanggal))}h)` : ` (sisa ${daysUntil(r.tanggal)}h)`);
  const group = (items, jenis) => items.filter((x) => x.jenis === jenis).map(line).join("\\n") || "(tidak ada)";
  const header = `<b>🔔 Ringkasan Notifikasi ASN</b>\\n${new Date().toLocaleString("id-ID")}\\n\\n`;
  const body = [
    `<b>Segera (≤ 90 hari)</b>`,
    `• <u>Kenaikan Gaji Berikutnya</u>`, group(soon, "Kenaikan Gaji Berikutnya"),
    `• <u>Kenaikan Pangkat Berikutnya</u>`, group(soon, "Kenaikan Pangkat Berikutnya"),
    "",
    `<b>Terlewat</b>`,
    `• <u>Kenaikan Gaji Berikutnya</u>`, group(overdue, "Kenaikan Gaji Berikutnya"),
    `• <u>Kenaikan Pangkat Berikutnya</u>`, group(overdue, "Kenaikan Pangkat Berikutnya"),
  ].join("\\n");
  return header + body;
}
...
function TelegramBox({ soon = [], overdue = [] }) {
  ...
  const sendNow = async () => {
    try {
      setBusy(true);
      const text = formatDigest({ soon, overdue });
      await sendTelegramMessage({ chatId: chatId.trim(), text });
  ...
  return (
    <Card title="Kirim Ringkasan ke Telegram" subtitle="Masukkan Chat ID dan kirim ringkasan notifikasi.">
      <div className="flex flex-col md:flex-row gap-3 items-start md:items-end">
        <div className="flex-1">
          ...
          <p className="text-xs text-slate-500 mt-1">Tip: kirim pesan ke bot, lalu dapatkan Chat ID via @userinfobot atau @RawDataBot.</p>
        </div>
      ...
      <div className="mt-3 text-xs text-slate-500">Untuk otomatis harian tanpa membuka aplikasi, butuh cron di Vercel. Lihat <code>TELEGRAM_SETUP.md</code>.</div>
    </Card>
  );
}
...
function FormRow({ label, required, children }) {
  return (<div><label className="block text-sm mb-1">{label} {required && <span className="text-rose-600">*</span>}</label>{children}</div>);
}
function NavButton({ icon, active, onClick, children }) {
  return (<button onClick={onClick} className={`px-3 py-1.5 rounded-lg border text-sm inline-flex items-center gap-2 ${active ? "bg-indigo-600 text-white border-indigo-600" : "bg-white hover:bg-slate-50"}`}>{icon}{children}</button>);
}
function TopLink({ active, onClick, icon, label }) {
  return (<button onClick={onClick} className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm ${active ? "bg-indigo-600 text-white border-indigo-600" : "bg-white hover:bg-slate-50"}`}>{icon}<span>{label}</span></button>);
}
function StatusPill({ label, target }) {
  ...
  return (<span className={`${base} bg-emerald-50 text-emerald-800 border-emerald-200`}><CheckCircle2 className="w-3 h-3" /> {label}: {d}h lagi</span>);
}
...
function ConfirmDialog({ open, title = "Konfirmasi", children, onConfirm, onCancel }) {
  if (!open) return null;
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 bg-black/30 grid place-items-center p-4 z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <motion.div initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 30, opacity: 0 }} className="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-200 p-6">
          ...
          <div className="flex justify-end gap-2">
            ...
            <button onClick={onConfirm} className="bg-indigo-600 text-white rounded-lg px-4 py-2 font-medium hover:bg-indigo-700">Ya, Simpan</button>
          </div>
        </motion.div>
      </motion.div>
    </AnimatePresence>
